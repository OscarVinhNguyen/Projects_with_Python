...
player_1 = Player(mode='player', color=(50, 50, 250), x=50, y=win_h/2-25, spd=5, cooldown = 30)
player_2 = Player(mode='player', color=(50, 175, 50), x=win_w-100, y=win_h/2-25, spd=5, cooldown = 30)
...
while run:
    ...
    if player_1_has_control:
        ...
        if player_1_shoot_timer <= 0:
            if keys[pygame.K_f]:
                bullets.append(Projectile(player_1.x+16, player_1.y+16, 1, 10, variety=0)) # rock
                player_1_shoot_timer = player_1.cooldown
            if keys[pygame.K_g]:
                bullets.append(Projectile(player_1.x+16, player_1.y+16, 1, 10, variety=1)) # paper
                player_1_shoot_timer = player_1.cooldown
            if keys[pygame.K_h]:
                bullets.append(Projectile(player_1.x+16, player_1.y+16, 1, 10, variety=2)) # scissors
                player_1_shoot_timer = player_1.cooldown
    ...
    if player_2_has_control:
        ...
        if player_2_shoot_timer <= 0:
            if keys[pygame.K_RSHIFT]:
                bullets.append(Projectile(player_2.x-16, player_2.y+16, -1, 10, variety=2)) # rock
                player_2_shoot_timer = player_2.cooldown
            if keys[pygame.K_RETURN]:
                bullets.append(Projectile(player_2.x-16, player_2.y+16, -1, 10, variety=1)) # paper
                player_2_shoot_timer = player_2.cooldown
            if keys[pygame.K_BACKSLASH]:
                bullets.append(Projectile(player_2.x-16, player_2.y+16, -1, 10, variety=0)) # scissors
                player_2_shoot_timer = player_2.cooldown
